...
describe("Test main groups visualization page", function() {
    ...
    it("Test depth first traversal with mock groups", testAsync(async () => {
        ...
        $.getScript('/src/main/webapp/scripts/groups-script.js', async function() {
            async function testDFS(groups, users, members) {
                var res = await loadGroupsDFS(groups[0], null, groups, users, members);
            ...
            var testDFSRes = await testDFS(groups, users, members);
            expect(data.name).toEqual("test-domain@domain.info");
            expect(data.children.length).toEqual(1);
            expect(data.children[0]).toBeDefined();
            expect(data.children[0]).not.toBeNull();
